...
module.exports = async function handler(req, res) {
  ...
  if (isMakeConfigured()) {
    ...
    makeSnapshot = {
      ...
      scenarios: [
        { nom: 'Propecto Social net', statut: 'DÉSACTIVÉ — 0 publication', detail: 'isActive:false, nextExec:null — rien n\'est programmé malgré les 120 posts prêts. Connexions Facebook/LinkedIn saines (pas expirées). Porte un flag "isinvalid" dont la cause précise reste à confirmer en tentant une réactivation dans Make (risque : ça republierait immédiatement en direct, à faire uniquement sur ton feu vert).' },
        { nom: 'Arbitrage Pro Social net', statut: 'DÉSACTIVÉ — 0 publication', detail: 'isActive:false, nextExec:null, ET le header d\'authentification contient toujours le texte "REMPLACER_PAR_CRON_SECRET" au lieu du vrai secret arbitragepro.eu — deux blocages cumulés, pas juste un secret à remplacer. Aucun fichier de contenu dédié Arbitrage+ trouvé par ailleurs (contrairement aux 3 autres projets).' },
        { nom: 'Pet Stone — Social Publisher', statut: 'ACTIF — fonctionne', detail: 'Créé le 21/08, tourne lun-ven 10h. Une seule exécution possible depuis sa création (lundi 24/08), et elle a réussi (1/1). Traite jusqu\'à 6 posts "Prêt" par jour ouvré — les 46 posts du calendrier seront tous publiés en ~1,5 semaine sans action requise.' },
        { nom: 'CVDesignPro — Publish social calendar', statut: 'ACTIF mais échoue à CHAQUE exécution', detail: 'Tourne bien lun-ven 12h depuis le 04/08, mais échoue systématiquement (dernière fois : 24/08 22h30) sur le module de téléchargement d\'image Google Drive — erreur "Unsupported alt type media for non byte stream". Vérifié : le fichier ciblé (ex. w01-tue-buzzwords.png) existe réellement dans Drive en image/png valide, donc la cause exacte reste à confirmer via "Run this module only" dans l\'éditeur Make (30 secondes, affiche l\'ID/mimetype exact reçu).' },
        { nom: '[Deals Social] 03 — Publication multi-canal', statut: 'à identifier', detail: 'Scénario découvert le 24/08 en creusant les connexions Facebook/LinkedIn — poste sur une page Facebook distincte (1262454503622951) via la table Supabase "arb_social_posts" et appelle aussi zernio.com/api/v1/posts. Ni "Deals" ni "Zernio" ne sont dans le registre de projets actuel — à clarifier avec Julien : sous-marque d\'Arbitrage+, ou projet distinct non suivi ?' },
        { nom: '[CE] A/B/C/D — Content Engine générique', statut: 'inactif', detail: 'Bloqué tant que la feuille PROJECTS du classeur Content Engine reste vide (xlsx pas importé) — backlog, déprioritisé par Julien le 24/08.' },
      ],
    };
  }
